...
-jUnit테스트(자바단위테스트): 하는목적은 jsp(컨트롤러) 힘빼기전에 CRUD테스트해보고 힘배라는 작업방식.
-jsp -> controller -> service -> dao ->mapper.xsml -> DB(Mysql,Oracle)
...
-위1가지로 마이바티스로 스프링에서 쿼리를 관리하는 처리 , sqlSessionFactiory (쿼리생성)
======================================================
-마이바티스에는 Select(), Insert(), Update(), Delete()쿼리를 수행하는 메서드들.
-위 1가지 Mybatis로 스프링에서 쿼리를 호출하는 메서드명(인터페이스)을 처리 sqlSessionTemplate(쿼리호출메서드명)
======================================================
...
###20201211(금)
-첨부파일 설정: servlet-context.xml에 파일 업로드 경로, 업로드 용량 제한 설정.
-위에 더해서 외부 의존성추가:pom.xml 메이븐 설정파일 내용 추가(직접타이핑x,가져옴). fileupload~.jar (내용추가하면 자동생성됨)
-RestAPI 댓글 컨트롤러 1개 만듦 -  네이버에서 RestAPI 로그인서버를 1개 만드는 것과 같음
-----------------------------------------------------------------------------
...
-admin폴더에는 크게 2가지 기능: 관리자관리(member폴더에), 게시물관리(board폴더에)
