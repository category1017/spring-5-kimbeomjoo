...
public class BoardDAOImpl implements IF_BoardDAO {
	...
	public List<BoardVO> selectboard(PageVO pageVO) throws Exception {
		// sql세션 템플릿(select,update,delete같은 메서드가 포함) 매퍼쿼리 지정(아래)
		// 매퍼쿼리를 (select,update,delete,insert등)을 실행할때, 개발자가 DB커넥션, 디스커넥션을 생각할 필요없이 
		// 사용가능한 메서드 집합을 구성해 놓은 것이 sql세션 템플릿이다.
	...
	public void updateBoard(BoardVO boardVO) throws Exception {
		// 게시물 수정 매퍼쿼리 연결(아래)
		// 위쪽의 메서드인 updateBoard메서드의 매개변수해석(아래) BoardVO클래스는 개발자생성한 참조형데이터타입.
		// jsp에서 update_form태그에서 전송된 값 boardVO클래스에 담겨서 데이터를 받습니다.
		// 함수는 오브젝트 생성해야지만, 사용이 가능합니다. 자바에선 new, 스프링에선 Inject
		// BoardVO 개발자 선언한 클래스, 데이터클래스, 오브젝트클래스, C언어-구조체클래스
		// String 자바가 선언한 클래스
