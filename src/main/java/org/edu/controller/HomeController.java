...
public class HomeController {
	...
	//전역 홈페이지에서 스프링 진입전 발생하는 에러페이지 처리
	...
	public String board_delete(RedirectAttributes rdat, @RequestParam("bno") Integer bno, @RequestParam("page") Integer page) throws Exception {
		//부모게시판에 첨부파일이 있다면 첨부파일 삭제처리 후 게시글 삭제(아래)
		List<AttachVO> delFiles = boardService.readAttach(bno);
		if(!delFiles.isEmpty()) {
			for(AttachVO file_name:delFiles) {//향상된 for 반복문
		...
		//DB에서 부모게시판에 댓글이 있다면 댓글삭제처리 후 게시글 삭제처리-서비스에 있음
		boardService.deleteBoard(bno);
		rdat.addFlashAttribute("msg", "삭제");//msg변수값은 URL에 표시가 나오지 않게 숨겨서 board_list보냅니다.
		return "redirect:/home/board/board_list?page="+page;//쿼리스트링 변수는 URL에 표시가 됩니다.
	}
		...
		for(MultipartFile file:files) {//여기의 file은 신규 저장하는 파일
			if(file.getOriginalFilename() !="") {
				...
				for(AttachVO file_name:delFiles) {//실제 폴더에서 기존 첨부파일 삭제처리
					if(index==sun) {
						File target = new File(commonController.getUploadPath(),file_name.getSave_file_name());//삭제할파일 경로
						if(target.exists()) {
				...
				real_file_names[index] =null;//신규파일 한글파일명 업로드
			}
			index += 1;
		}
		boardVO.setSave_file_names(save_file_names);
	...
	public String board_write(RedirectAttributes rdat,@RequestParam("file") MultipartFile[] files, BoardVO boardVO) throws Exception {
		...
		for(MultipartFile file:files) {
			...
			real_file_names[index] = file.getOriginalFilename();//파일의 진짜 이름 예를들면 한글파일명
			}
			index =  index + 1;
		}
		...
		return "redirect:/home/board/board_list";//실제 DB에 인서트
	}
	...
	public String board_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception{
		...
		pageVO.setPerPageNum(5);//페이지 하단 페이징 번호개수
	...
	public String mypage_update(HttpServletRequest request, MemberVO memberVO, RedirectAttributes rdat) throws Exception {
		...
		rdat.addFlashAttribute("msg", "회원수정");//model로 값을 보내지 못하는 이유는 redirect이기 때문에
		return "redirect:/member/mypage";
	}
	//사용자 홈페이지 회원 마이페이지 접근 매핑
	@RequestMapping(value="/member/mypage",method=RequestMethod.GET)
	public String mypage(HttpServletRequest request, Model model) throws Exception{
		//마이페이지는 로그인 상태만 접근가능하기 때문에, 로그인 세션변수중 로그인아이디 변수 session_userid를 사용(login Controller확인)
		HttpSession session = request.getSession();
		...
		for(BoardVO boardVO:board_list) {//board_list변수에는 최대 5개의 레코드가 존재함.  
			...
			if(file_list.size() == 0) {//첨부파일이 없을때
				...
				for(AttachVO file_name:file_list) {
					...
					if(imgCheck) {//첨부파일이 이미지 일때
						save_file_names[cnt] = save_file_name;
						//System.out.println("디버그["+cnt+"]"+save_file_names[cnt]);
						break;//이중 반복문에서 현재for문만 종료 
					}else {//첨부파일이 이미지가 아닐때 ex,엑셀 한글파일
						save_file_names[cnt] = "";
						//System.out.println("디버그["+cnt+"]"+save_file_names[cnt]);
					}
				}
