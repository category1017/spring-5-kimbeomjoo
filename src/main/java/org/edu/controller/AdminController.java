...
public class AdminController {
	...
	@Inject
	IF_MemberService memberService;//멤버인터페이스를 주입빋아서 memberService오브젝트 변수를 생성.
	...
	public String bbs_type_delete(BoardTypeVO boardTypeVO, RedirectAttributes rdat) throws Exception{
		...
		if(board_count > 0) {
			rdat.addFlashAttribute("msg_fail", "해당게시판에 게시물내용이 존재합니다. 삭제");
	...
	public String bbs_type_list() throws Exception{
		//여기는 model을 이용해서 jsp로 board_type_lsit오브젝트를 보낼필요X, ControllerAdvice클래스에서 만들었기 때문에
	...
	public String board_update(@RequestParam("bno") Integer bno,@ModelAttribute("pageVO")PageVO pageVO,Model model) throws Exception {
		...
		*/
		...
		//시큐어 코딩 적용(아래) jsp에서 c:out jstl로 대체
	...
	public String board_update(RedirectAttributes rdat,@RequestParam("file") MultipartFile[] files, BoardVO boardVO, PageVO pageVO) throws Exception{
		...
		for(MultipartFile file:files) {//다중파일 업로드 호출 부분 시작 향상된 for문 사용
			if(file.getOriginalFilename() != "" ) {//첨부파일명이 있으면
				...
				int sun = 0;//수정update.jsp화면에서 첨부파일을 개별 삭제시 사용할 순서가 필요하기 때문 변수 추가
				//기존파일 폴더에서 실제파일 삭제 처리
				for(AttachVO file_name:delFiles) {
					if(index == sun) {//index는 첨부파일 개수, sun삭제할 개별순서
						File target = new File(commonController.getUploadPath(), file_name.getSave_file_name());
						if(target.exists()) {
					...
					sun = sun + 1;//개별삭제는 for문에서 딱1번 뿐이기 때문에
				}
				//신규파일 폴더에 업로드 처리 
	...
	public String board_write(RedirectAttributes rdat, @RequestParam("file")MultipartFile[] files, BoardVO boardVO) throws Exception {
		//post받은 boardVO(바인딩된)내용을 DB서비스에 입력하면 됨.
		//DB에 입력후 새로고침 명령으로 게시물 테러를 당하지 않으려면, redirect로 이동처리 합니다.(아래)
	...
	public String board_view(@ModelAttribute("pageVO") PageVO pageVO, @RequestParam("bno") Integer bno, Model model) throws Exception {
		 ...
		 * "첫번째 내용 입니다.<br><br>줄바꿈 처리입니다. <script>loacation.href('이상한사이트로 이동');</script>"
		...
		boardVO.setContent(securityCode.unscript(xss_data));
		//시큐어코딩 끝
		//첨부파일 리스트 값을 가져와서 세로데이터(jsp에서는 forEach문 사용)를 가로데이터(jsp에서 배열사용)로 바꾸기
		...
		/*
		for(HashMap<String, Object> file_name:files_noUse) {
			save_file_names[cnt] = (String) file_name.get("save_file_name");//형변환 cast
			real_file_names[cnt] = (String) file_name.get("real_file_name");
			cnt = cnt +1;
		}
		*/
		...
		boardVO.setSave_file_names(save_file_names);
		boardVO.setReal_file_names(real_file_names);
		//위처럼 첨부파일을 세로배치->가로배치로바꾸고 get/set하는 이유는 attachVO를 만들지 않아서 입니다. 
	...
	public String board_list(@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		...
		*/
		 ...
		 * input_board.setContent("첫번째 내용입니다.<br>줄바꿈 했습니다.");
		 ...
		 * //input_board = {1,"첫번째 게시물 입니다.", "첫번째 내용입니다.<br>줄바꿈했습니다.", "admin",
		 * now(),2,0}; board_array[0] = input_board;
		 * //-------------------------------------------------- BoardVO input_board2 =
		 * new BoardVO(); input_board2.setBno(2); input_board2.setTitle("두번째 게시물 입니다.");
		 * input_board2.setContent("두번째 내용입니다.<br>줄바꿈 했습니다.");
		 * input_board2.setWriter("user02"); input_board2.setReg_date(reg_date);
		 * input_board2.setView_count(2); input_board2.setReply_count(0); //input_board
		 * = {1,"첫번째 게시물 입니다.", "첫번째 내용입니다.<br>줄바꿈했습니다.", "admin", now(),2,0};
		 * board_array[1] = input_board2; //-----------------------------------------
		 * List<BoardVO> board_list = Arrays.asList(board_array);//배열타입을 List타입으로 변경하는
		 * 절차.
		 */		
		//selectMember마이바티스쿼리를 실행하기 전에 set이 발생해야 변수값이 할당됩니다.(아래)
		//PageVO의 queryStartNo구하는 계산식 먼저 실행되어서 변수값이 발생되어야함.
		if(pageVO.getPage() == null) {//int 일때  null체크에러가 나와서 pageVO의 page변수형 Integer로 변경.
			pageVO.setPage(1);
		}
		pageVO.setPerPageNum(8);//리스트하단에 보이는 페이징번호의 갯수
		...
		pageVO.setTotalCount(countBoard);// 11x개 현재 전체 게시물 수를 구한 변수 값 입력 매개변수로 입력하는 순간 calcPage()메서드실행.
		
		List<BoardVO> board_list = boardService.selectBoard(pageVO);
		model.addAttribute("board_list", board_list);
		//model.addAttribute("pageVO", pageVO); //@ModelAttribute 에노테이션으로 대체
		return "admin/board/board_list";
	}
	
	//메서드 오버로딩(예, 동영상 로딩중..., 로딩된 매개변수가 다르면 메서드이름을 중복사용가능합니다.)
	@RequestMapping(value="/admin/member/member_write",method=RequestMethod.POST)
	public String member_write(@Valid MemberVO memberVO) throws Exception{
		//아래 GET방식의 폼출력화면에서 데이터 전송받은 내용을 처리하는 바인딩.
		...
		return "redirect:/admin/member/member_list";//절대경로로 처리된 이후에 이동할 URL주소를 여기에 반환 (글쓰기 후 새로고침 방지)
	}
	...
	public String member_update(PageVO pageVO,@Valid MemberVO memberVO) throws Exception {
		...
		//POST 방식으로 넘어온 값을 DB수정처리하는 역할
		memberService.updateMember(memberVO);
		//redirect를 사용하는 목적은 새로고침 했을때, 위 updateMember메서드를 재실행 방지 목적임.
	...
	public String member_delete(RedirectAttributes rdat,@RequestParam("user_id") String user_id) throws Exception {
		//Redirect로 페이지 이동시 전송값을 숨겨서 보내는 역할 클래스 RedirectAttributes
		memberService.deleteMember(user_id);
		rdat.addFlashAttribute("msg","삭제");
		return "redirect:/admin/member/member_list";//?success=ok 로 하면 보안상의 위험이 있으므로 위와 같은 클래스를 씀.
	}
	//member_list.jsp에서 보낸 데이터를 수신하는 역할 @RequestParam("키 이름") 리퀘스트파라미터 클래스 사용
	//현재컨트롤러 클래스에서 jsp로 데이터를 보내는 역할 Model 클래스 사용. 
	//데이터 흐름 : member_list > @RequestParam("user_id")수신, Model 송신 > member_view.jsp
	...
	public String member_view(@ModelAttribute("pageVO") PageVO pageVO, @RequestParam("user_id") String user_id, Model model) throws Exception{
		...
		//(member_list에서 받은 user_id를 member_view에 보냄)
		//member_view.jsp에서 model로 수신한 데이터user_id2를 출력하는 방법
		MemberVO memberVO = memberService.readMember(user_id);
		model.addAttribute("memberVO", memberVO);
	...
	public String member_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception {
		 ...
		 * "ROLE_ADMIN"},
		 * {"user","일반사용자1","user@abc.com","false","2020-12-04","ROLE_USER"} };
		 * //{"user_id":"admin","user_name":"관리자",...} 해시데이터# (그물-낚시)=표데이터 //아래 Object는
		 * java.lang패키지의 최상위 클래스로서 import 안해도 자동으로 사용가능. //Map 타입이 부모, HashMap 자식클래스
		 * 이런방식으로 사용하는 것이 관례상, paramMap오브젝트의 확장하기 편하도록 하기 위해서 //Map타입을 상속받아서, HashMap타입의
		 * 오브젝트를 생성하는 방식. Map<String,Integer> mapTest = new HashMap<String,Integer>();
		 * String ageValue = "40"; int ageValue2 = 40; mapTest.put("ageValue2",
		 * ageValue2); mapTest.put("age",Integer.parseInt(ageValue));//제네릭타입을 사용하면, 여기처럼
		 * parseInt형변환을 할필요가 없음 //제네릭타입의 근본목적은 데이터타입에 대해서 명시적인 코딩을 해서 코드를 단순화 시키기 위해서
		 * 
		 ...
		 * 배열 오브젝트 생성 [2]는 배열의 크기(=레코드갯수) members_array[0] = members_input;
		 * members_array[1] = members_input;
		 * //-----------------------------------------------------------------------
		 * //실제 코딩에서는 배열타입으로 보내지 않고, List타입(목록)으로 model 이용해서 jsp로 보냄. List<MemberVO>
		 * members_list = Arrays.asList(members_array); List m_list = null; //위에서 만든
		 * members_array배열오브젝트를 Arrays.as.List타입으로 변경해서 jsp로 보냄. //위에서 데이터타입연습으로 총3가지
