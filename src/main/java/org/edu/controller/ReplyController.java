...
public class ReplyController {
	...
	public ResponseEntity<Map<String,Object>> reply_list(@PathVariable("bno") Integer bno, @PathVariable("page") Integer page) throws Exception{
		...
		pageVO.setPage(page);//조건은 Ajax로 호출시 항상page변수는 반드시 보내야 합니다.
		pageVO.setPerPageNum(3);//페이지 하단에 보이는 페이징번호의 개수
		pageVO.setQueryPerPageNum(5);//댓글 1페이지당 보여줄 댓글 개수
		int replyCount = replyDAO.selectReplyCount(bno);
		pageVO.setTotalCount(replyCount);//전체 댓글 개수 구해서 set하는 순간이 필수 prev,next구할때 필요.
				
		//페이지 계산식 처리 끝
		//현재 게시물에 달린 댓글 전체개수 구하기 : 게시물관리테이블에 있는 reply_count를 가져다가 사용
	...
	public ResponseEntity<String> reply_write(@RequestBody ReplyVO replyVO) {
		...
		ResponseEntity<String> result = null;
		try {//예외처리를 상위 메서드로 보내지 않는 이유는 RestAPI에서 예외 메세지를 개발자가 제공하기 위해서
			replyDAO.insertReply(replyVO);
			result = new ResponseEntity<String>("suscess", HttpStatus.OK);
			replyDAO.updateCountReply(replyVO.getBno());
						
		} catch (Exception e) {
			result = new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
		}
		return result;//ResponseEntity객체클래스형 String값을 ajax로 호출한 페이지로 반환.
	}
