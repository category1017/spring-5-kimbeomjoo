...
public class CommonController {
	 ...
	 * 사용용도1 : 게시물상세보기 첨부파일이 이미지면 미리보기 이미지가 보이도록, 이미지가 아니면, 다운로드링크만 보이도록
	...
	fileArray = baos.toByteArray();//바이트 단위로 되어있는 변수에 아웃풋스트림내용을 저장해서 return 으로 반환
	fis.close();//고전 자바프로그램에서는 메모리 관리를 위해서 fis파일 인풋스트림변수 생성후 소멸시키는 작업이 필수
	baos.close();//스프링프레임워크 기반의 프로그램구조에서는 close와 같은 메모리 관리를 할 필요가 없습니다.= 스프링은 가비지컬렉트 기능이 내장
	
	final HttpHeaders headers = new HttpHeaders();//크롬 개발자 도구 > 네트워크> image_preview클릭 >헤더탭 확인가능
	String ext = FilenameUtils.getExtension(save_file_name);//파일 확장자 구하기
	switch(ext.toLowerCase()) {//확장자 소문자로 변경후 switch case문으로 분리
	case "png":
	...
	//REST-API서비스로 사용할 때, @ResponseBody 애노테이션으로 json텍스트데이터를 반환함.(아래)
	//아래는 RestAPI백엔드단, Ajax(jsp) Rest-API의의 프론트엔드단. 
	@RequestMapping(value="/id_check", method=RequestMethod.GET)
	@ResponseBody
	public String id_check(@RequestParam("user_id") String user_id) {
		String result = "0";//아이디 중복값을 체크하는 변수 초기값은 중복값 없음.
		//Rest-API 서비스에서는 스프링을 통해서 Ajax로 에러메세지를 받을수 없기 때문에 여기서 에러를 처리해야함.
		try {
