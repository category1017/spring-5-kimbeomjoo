...
<!-- 댓글리스트 템플릿(빵틀) 만들기(아래) -->
<!-- jsp의 forEach문과 같은 역할  items="${members}" var="member"> 같은 역할 {{#each .}} -->
...
var printPageVO = function(pageVO,target){
	...
	for(var cnt=pageVO.startPage ; cnt<=pageVO.endPage;cnt++){ var active = (cnt==pageVO.page)?"active":"";
...
var replyList = function(){
	...
	$.ajax({
		url:"/reply/reply_list/${boardVO.bno}/"+page,//쿼리스트링으로 안보내고, 패스베리어블로 보냅니다. 
		type:"post",//원래는 get인데,post로 보낼수있음
		dataType:"json",
	...
	$("#updateReplyBtn").on("click",function(){
		...
		$.ajax({
			...
			success:function(result){
				...
				replyList();//댓글 수정 후 댓글리스트 재호출(실행)
				}else{
...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		if("${session_enabled}" == ""){//버튼클릭시 비로그인시 로그인 화면으로 유도
			...
			location.href="/login";//자바스크립트 내장함수
			return false;
		}
		//alert("디버그");
