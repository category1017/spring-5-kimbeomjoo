...
<script>
//아이디 기존회원아이디가 존재하는지 Ajax확인후(RestAPI사용) 
//신규아이디일때만 [회원등록 버튼] 활성화 시키기, 초기엔 비활성화 시키기
jQuery(document).ready(function(){ 
	//$대신에 jQuery사용하는 목적, 자바변수$와 헷갈리는 것을 방지 효과.
	//목적2.PHP기반(워드프레스나,카페24쇼핑몰 같은 솔루션에서 제이쿼리를 사용할때) $사용하면 작동이 안됨.-> jQuery로 사용해야한다. 
	jQuery("#btn_join").attr("disabled",true); //초기에는 서밋버튼 비활성화
	jQuery("#btn_join").css("opacity","0.5"); //#은 엘리먼트의 id를 선택할때 사용,  .은 엘리먼트의 class를 선택시 사용
	jQuery("input[name='user_id']").bind("blur",function(){
		//blur이벤트는 focus선택과는 반대이벤트=선택을 벗어났을때 이벤트
...
jQuery(document).ready(function(){
	jQuery("form[name='join_form']").on("submit",function(event){
		event.preventDefault();//서밋이 발생시 기본전송 중지 아래 내용 실행
