...
$(document).ready(function(){
	...
	var current_2 = current.split("/")[2];//split함수로 current에 있는 문자를 분리한 배열값을 변환.
	//alert(current_2);//디버그용으로 출력
	//alert(current);//admin/(대시보드-undefined),admin/member_list(관리자관리),
	//alert/board_list(게시물관리)
	//active 클래스명을 동적으로 추가할 영역은  nav-item 안쪽의 a태그의 클래스명을 추가하면됨.
		$(".nav-treeview li a").each(function() {
		if( $(this).attr('href'). indexOf(current_2) != -1 ) {
			//위 문제를 처리하는 대체 함수 indexOf 사용
			if(current_2 != 'board'){
				$(this).addClass("active");//선택한 메뉴의 배경을 흰색으로 보이게 추가하는 처리.
			}
		} else {
			if(current_2 != 'board'){
				$(this).removeClass("active");//선택하지 않은 메뉴의 배경색 흰색을 제거하는 처리.
			}
		}
	});	
});
/*each함수로 a태그 2개를 찾는 명령. 결과는 2개이기 때문에 2번 반복
$(this) 현재 함수의 구현 대상 본인을 가리킵니다.
2개의 값을 비교1: - a태그의 값 /admin/member_list, /admin/board_list
2개의 값을 비교2: - 비교대상 current_2(현재 웹브라우저의 URL값 중 제일 마지막 값)
if( $(this).attr('href').includes(current_2) == true ) {includes 포함하고 있는 문자열을 비교.
	위에서 사용한 includes함수는 큼롬에서만 작동이 되어서 IE에서는 작동이 안됨. 크로스브라우징처리를 해줘야 함.*/
