			...
			<div class="row">
				...
				<div class="col-12">
					...
					<form name="update_form" action="/admin/board/board_update" method="post" encType="multipart/form-data">
							...
							<div class="card-body">
								<div class="form-group">
									...
									<input type="text" value="${boardVO.title}"
										class="form-control" name="title" id="title" placeholder="제목을 입력해주세요." required>
								...
								<div class="form-group">
									...
									<textarea rows="5" name="content" id="content"
										class="form-control"><c:out value= "${boardVO.content}"/></textarea>
								...
								<c:forEach var="index" begin="0" end="1">
									...
									<c:if test="${boardVO.save_file_names[index] !=null}">
											...
											<strong><i class="fas fa-save mr-1"></i>첨부파일${index}</strong>
											<p class="text-muted">
						...
						<!-- 버튼영역 시작 -->
						<div class="card-body">
							<a href="/admin/board/board_view?page=${pageVO.page}&bno=${boardVO.bno}" class="btn btn-primary float-right mr-1">뷰화면 이동</a>
			...
			//alert("디버그 : 삭제할 파일명은 "+ save_file_name); return false;
				$.ajax({
					...
					success:function(result){
						if(result=="success") {//실제 파일+DB테이블 삭제 후 화면에서도 삭제처리(아래)
							click_element.parents(".div_file_delete").remove();
						}
					},
					error:function(result){
						alert("RestAPI 접근에 실패하였습니다." );
