...
public class LoopApp {
	...
	public static void main(String[] args) {
		...
		//for문 (반복의 시작 값 정의; 반복의 종료 값 정의; 반복할 때 증가하는 값) {반복할때 구현내용}
		for(cnt=0;cnt<10;cnt++) {//cnt++ ==>  cnt=cnt+1; 의미 1씩 증가하는 의미
			...
			//int length = names.length;//length 기본형 변수에는 5가 들어가게 됨.
			//names는 클래스형 변수이지만, 인스턴스는 아닙니다. 
			char name='사'; //임베디드장비와 자바프로그램 통신시 이것을 사용
			//임베디드 프로그램에서 char을 사용하는 이유는 메모리를 효율적으로 사용하기 위해서 char 사용. 메모리 크기가 작기 때문.
			//자바에서 string형 큰따옴표는 문자열데이터 감싸줄때, char형 작은따옴표는 글자1개인 문자데이터를 감싸줄때
			...
			//향상된for문, 회원정보를 jsp로 출력하기 현장과 비슷하게 구현
			String[][] members = {
