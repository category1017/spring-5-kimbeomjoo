...
public class OracleDriverTest {
	...
	public static void main(String[] args) {
		...
		//현재 디펜던스로 pom.xml(메이븐),Externa jar(직접)등록된 jdbc는 몇개가 있는지 확인
		//예를들면 개발자가 등록한 다음 3개 확인 ojdbc6, mysql, hsql
		//DriverManager클래스는 JDBC드라이버 인스턴스(실행클래스)를 관리하는 클래스
		//.getDrivers()메서드를 이용해서 현재 등록된 드라이버를 뽑아올수 있음.
		//데이터타입 중 이넘(반복되는 형태의 자료)Enumerate 
